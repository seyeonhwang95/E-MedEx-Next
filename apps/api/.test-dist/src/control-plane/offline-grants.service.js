...
let OfflineGrantsService = class OfflineGrantsService {
    offlineDeviceGrantRepository;
    tenantRepositoryResolver;
    constructor(offlineDeviceGrantRepository, tenantRepositoryResolver) {
        this.offlineDeviceGrantRepository = offlineDeviceGrantRepository;
        this.tenantRepositoryResolver = tenantRepositoryResolver;
    }
    async listOfflineGrants(query) {
    ...
    async listOfflineGrantAuditEvents(grantId, query) {
        ...
        const filteredItems = items.filter((item) => {
            ...
            const eventAtTimestamp = eventAtRaw instanceof Date ? eventAtRaw.getTime() : eventAtRaw ? Date.parse(String(eventAtRaw)) : Number.NaN;
        ...
        filteredItems.sort((left, right) => {
            const leftRaw = left.eventAt;
            const rightRaw = right.eventAt;
        ...
        if (query.cursor !== undefined) {
            ...
            const lastItem = cursorItems[cursorItems.length - 1];
    ...
    async enrollOfflineGrant(httpRequest, request) {
        ...
        const saved = await this.offlineDeviceGrantRepository.save(this.offlineDeviceGrantRepository.create({
            ...(existing ?? {}),
            grantId: randomUUID(),
            tenantId,
            userId: request.userId,
    ...
    async validateOfflineGrant(httpRequest, request) {
        const tenantId = request.tenantId.toLowerCase();
        const scope = request.scope ?? 'FieldIntake';
    ...
    async revokeOfflineGrant(httpRequest, request) {
        const tenantId = request.tenantId.toLowerCase();
        const record = await this.offlineDeviceGrantRepository.findOne({
            where: {
                grantId: request.grantId,
    ...
    async acknowledgeOfflineGrantWipe(httpRequest, request) {
        const tenantId = request.tenantId.toLowerCase();
        const scope = request.scope ?? 'FieldIntake';
        const record = await this.offlineDeviceGrantRepository.findOne({
            where: {
    ...
    buildTenantContext(tenantIdInput) {
...
OfflineGrantsService = __decorate([
    Injectable(),
    __param(0, InjectRepository(OfflineDeviceGrantEntity)),
    __param(1, Inject(TENANT_REPOSITORY_RESOLVER)),
    __metadata("design:paramtypes", [Function, Object])
], OfflineGrantsService);
export { OfflineGrantsService };
