    ...
    async find(options) {
        ...
        if (orderEntry) {
            ...
            items = items.sort((left, right) => {
                ...
                const leftValue = leftRaw instanceof Date ? leftRaw.getTime() : typeof leftRaw === 'number' ? leftRaw : String(leftRaw);
                const rightValue = rightRaw instanceof Date ? rightRaw.getTime() : typeof rightRaw === 'number' ? rightRaw : String(rightRaw);
...
describe('API integration', () => {
    ...
    it('deduplicates hl7 message ingestion per tenant', async () => {
        const rawMessage = 'MSH|^~\\&|LAB|TOX|EMEDEX|DEMO|20260316100500||ORU^R01|ORU-10001|P|2.5.1\rPID|1||123456^^^EMEDEX||DOE^JANE';
    ...
    it('reconciles unmatched results by matching to tenant case and supports rejection', async () => {
        ...
        const rawMessageOne = 'MSH|^~\\&|LAB|TOX|EMEDEX|DEMO|20260316101500||ORU^R01|ORU-20001|P|2.5.1\rPID|1||123456^^^EMEDEX||DOE^JOHN';
        const rawMessageTwo = 'MSH|^~\\&|LAB|TOX|EMEDEX|DEMO|20260316102500||ORU^R01|ORU-20002|P|2.5.1\rPID|1||987654^^^EMEDEX||ROE^JANE';
    ...
    it('prevents cross-tenant match for unmatched results', async () => {
        ...
        const rawMessage = 'MSH|^~\\&|LAB|TOX|EMEDEX|DEMO|20260316103500||ORU^R01|ORU-20003|P|2.5.1\rPID|1||555555^^^EMEDEX||CROSS^TENANT';
        await request(app.getHttpServer()).post('/labs/hl7/messages').set('x-tenant-id', 'demo').send({
            labCode: 'LAB-C',
            direction: 'Inbound',
            rawMessage,
