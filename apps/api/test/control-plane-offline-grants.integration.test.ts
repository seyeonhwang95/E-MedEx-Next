...
class InMemoryRepository {
  ...
  private matchesWhere(item: Record<string, unknown>, where?: Record<string, unknown>) {
    if (!where) {
      return true;
    }
...
describe('Control-plane offline grant lifecycle integration', () => {
  ...
  const offlineGrantStore: InMemoryRow[] = [];
  const auditEventStore: InMemoryRow[] = [];
  ...
  before(async () => {
    ...
    app.use((req: Request, _res: Response, next: NextFunction) => {
      ...
      requestWithAuth.tenant = {
        tenantId: String((req.body as Record<string, unknown> | undefined)?.tenantId ?? 'demo').toLowerCase(),
        subdomain: 'demo',
  ...
  it('enrolls an offline grant and validates as active', async () => {
    ...
    const enrolledAuditEvent = auditEventStore[auditEventStore.length - 1] as Record<string, unknown>;
    ...
    const validatedAuditEvent = auditEventStore[auditEventStore.length - 1] as Record<string, unknown>;
  ...
  it('revokes a grant and returns wipe-required on validation', async () => {
    ...
    const revokedAuditEvent = auditEventStore[auditEventStore.length - 1] as Record<string, unknown>;
  ...
  it('acknowledges wipe and clears wipeRequired for revoked grant', async () => {
    ...
    const latestAuditEvent = auditEventStore[auditEventStore.length - 1] as Record<string, unknown>;
  ...
  it('lists offline grant audit events for a grant and tenant', async () => {
    ...
    const auditListResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&page=1&pageSize=20`,
    );
    ...
    const eventTypes = (auditListResponse.body.items as Array<{ eventType: string }>).map((item) => item.eventType);
    ...
    const filteredAuditListResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&eventType=offline_grant_validated&page=1&pageSize=20`,
    );

    assert.equal(filteredAuditListResponse.status, 200);
    assert.equal(filteredAuditListResponse.body.total >= 1, true);
    const filteredEventTypes = (filteredAuditListResponse.body.items as Array<{ eventType: string }>).map(
      (item) => item.eventType,
    );
    assert.equal(filteredEventTypes.every((eventType) => eventType === 'offline_grant_validated'), true);

    const enrolledAudit = auditEventStore.find(
      (item) => item.targetId === enrollResponse.body.grantId && item.eventType === 'offline_grant_enrolled',
    );
    const validatedAudit = auditEventStore.find(
      (item) => item.targetId === enrollResponse.body.grantId && item.eventType === 'offline_grant_validated',
    );
    const revokedAudit = auditEventStore.find(
      (item) => item.targetId === enrollResponse.body.grantId && item.eventType === 'offline_grant_revoked',
    );
    ...
    const timeWindowResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&from=2026-03-16T09:30:00.000Z&to=2026-03-16T10:30:00.000Z&page=1&pageSize=20`,
    );
    ...
    const sortedAscendingResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&from=2026-03-16T08:30:00.000Z&to=2026-03-16T11:30:00.000Z&sort=eventAt:asc&page=1&pageSize=20`,
    );

    assert.equal(sortedAscendingResponse.status, 200);
    const sortedEventTypes = (sortedAscendingResponse.body.items as Array<{ eventType: string }>).map(
      (item) => item.eventType,
    );
    ...
    const firstCursorPage = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&sort=eventAt:desc&pageSize=2&cursor=`,
    );
    ...
    const secondCursorPage = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&sort=eventAt:desc&pageSize=2&cursor=${encodeURIComponent(String(firstCursorPage.body.nextCursor))}`,
    );

    assert.equal(secondCursorPage.status, 200);
    assert.equal(secondCursorPage.body.items.length >= 1, true);

    const firstPageIds = (firstCursorPage.body.items as Array<{ id: string }>).map((item) => item.id);
    const secondPageIds = (secondCursorPage.body.items as Array<{ id: string }>).map((item) => item.id);
    const overlap = secondPageIds.some((id) => firstPageIds.includes(id));
    assert.equal(overlap, false);

    const malformedCursorResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&sort=eventAt:desc&pageSize=20&cursor=not-a-valid-cursor`,
    );
    ...
    const staleCursor = Buffer.from(
      JSON.stringify({ id: 'missing-audit-id', eventAt: '2026-03-16T10:00:00.000Z' }),
      'utf8',
    ).toString('base64url');
    const staleCursorResponse = await request(app.getHttpServer()).get(
      `/control/offline-grants/${enrollResponse.body.grantId}/audit-events?tenantId=demo&sort=eventAt:desc&pageSize=20&cursor=${encodeURIComponent(staleCursor)}`,
    );
